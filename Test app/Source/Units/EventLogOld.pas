...
class procedure TEventLogOld.WriteInfo(AMessage: string);
begin
  Write(EVENTLOG_INFORMATION_TYPE, 2, AMessage);
end;

class procedure TEventLogOld.WriteWarning(AMessage: string);
begin
  Write(EVENTLOG_WARNING_TYPE, 3, AMessage);
end;

class procedure TEventLogOld.WriteError(AMessage: string);
begin
  Write(EVENTLOG_ERROR_TYPE, 4, AMessage);
end;

class procedure TEventLogOld.Write(AEntryType: Word; AEventId: Cardinal; AMessage: string);
var
  h: THandle;
begin
...
class procedure TEventLogOld.AddEventSourceToRegistry;
var
  reg: TRegistry;
begin
...
TEventLogOld.Source := 'My Application Name';
